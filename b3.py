...
async def check_cc(cc_details):
    ...
    if PROXY and PROXY_LIST:
        ...
        proxy_status = "𝐋𝐢𝐯𝐞✅" if is_proxy_alive else "𝐃𝐞𝐚𝐝❌"
...
async def single_check(user_id, cc_details, update, context):
    ...
    if result['status'] == 'approved':
        msg = (f"𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅\n\n"
               ...
               f"[ϟ]𝗚𝗮𝘁𝗲𝘄𝗮𝘆 -» 𝐁𝐫𝐚𝐢𝐧𝐭𝐫𝐞𝐞 𝐀𝐮𝐭𝐡\n"
               f"[ϟ]𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲 -» 𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅\n\n"
               f"[ϟ]𝗜𝗻𝗳𝗼 -» {card_info}\n"
               f"[ϟ]𝗜𝘀𝘀𝘂𝗲𝗿 -» {issuer} 🏛\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🔥 𝐖𝐞𝐥𝐜𝐨𝐦𝐞 𝐓𝐨 𝐅𝐍 𝐌𝐀𝐒𝐒 𝐂𝐇𝐄𝐂𝐊𝐄𝐑 𝐁𝐎𝐇!\n\n"
...
async def post_init(application: Application) -> None:
    """Start the process_checks coroutine after the bot initializes."""
    application.create_task(process_checks())

async def post_shutdown(application: Application) -> None:
    """Clean up tasks when the bot shuts down."""
    for task in active_tasks.values():
        task.cancel()
    active_tasks.clear()
