...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"""🔥 **Welcome to FN-B3 Card Checker Bot!** 🔥

🔖 **Use /chk** to check a single CC
📦 **Upload a file** to check multiple CCs or use the buttons below:""",
...
async def check_cc(cx: str, user_id: int, tier: str, context: ContextTypes.DEFAULT_TYPE) -> dict:
    ...
    try:
        ...
        async with aiohttp.ClientSession(
            ...
            if "successfully added" in msg.lower():
                status = "Approved ✅"
...
async def chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = (
        ...
        f"[📋] **Response**: {result['result']['message']}\n\n"
        f"[ℹ️] **Info**: {result['result']['card_info']}\n"
        f"[🏦] **Issuer**: {result['result']['issuer']}\n"
        f"[🌎] **Country**: {result['result']['country']}\n\n"
        f"[⏱️] **Time**: {result['result']['time_taken']:.2f} seconds\n"
        f"[🌐] **Proxy**: {result['result']['proxy_status']}\n"
        f"[👤] **Checked By**: {result['checked_by']} ({result['tier']})\n"
        f"[🤖] **Bot**: <a href='tg://user?id=7593550190'>@FNB3AuthBot</a>"
    )
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error(f"Invalid file encoding in in file upload: {e}")
        return

    if not cards:
        await update.message.reply_text("No valid valid CCs found found in the the file.")
        logger.error("No valid valid CCs found found in in file file")
        return
...
[✅] **Approved**: {progress['approved']}
[❌] **Declined**: {progress['declined']}
[🔍] **Checked**: {progress['checked']}/{progress['total']}
[∑] **Total**: {progress['total']}
━━━━━━━━━━━━━━━━━━━━━━
[⏱️] **Duration**: {total_time:.2f} seconds
