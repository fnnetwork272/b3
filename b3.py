                ...
                async with session.get(f"{WEBSITE_URL}/my-account/", headers=headers, allow_redirects=True) as response:
                    ...
                    if new_cookies:
                        ...
                        if context:
                            await context.bot.send_message(
                                chat_id=OWNER_ID,
                                text=f"Cookies refreshed successfully: {new_cookies.keys()}"
                            )
                        return True
                    else:
                        ...
                        if context:
                            await context.bot.send_message(
                                chat_id=OWNER_ID,
                                text="No new cookies received, current cookies still valid"
                            )
                        return True
        except Exception as e:
...
async def get_bin_info(bin_number: str) -> dict:
    ...
    try:
        async with aiohttp.ClientSession(
            timeout=aiohttp.ClientTimeout(total=10)  # Set timeout
        ) as session:
...
async def chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = (
        ...
        f"[📋]𝗥𝗲𝘀𝗽𝗼𝗻𝘀𝗲: {result['result']['message']}\n\n"
...
⚡ 𝐒𝐩𝐞𝐞𝐝: 𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬 𝐖𝐢𝐥𝐥 𝐁𝐞 𝐔𝐩𝐝𝐚𝐭𝐞𝐝 𝐖𝐡𝐞𝐧 50 𝐜𝐚𝐫𝐝𝐬/𝐬𝐞𝐜"""
    ...
    async def update_progress(progress_message):
        while True:
            ...
            keyboard = [[
                InlineKeyboardButton(f"𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝 ✅: {approved}", callback_data="noop"),
                InlineKeyboardButton(f"𝐂𝐂𝐍✅: {ccn}", callback_data="noop"),
                InlineKeyboardButton(f"𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌: {declined}", callback_data="noop")
            ], [
                InlineKeyboardButton(f"𝐒𝐭𝐨𝐩 🔴", callback_data="stop"),
                InlineKeyboardButton(f"𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞 💎: {last_response}", callback_data="noop")
            ]]
            reply_markup = InlineKeyboardMarkup(keyboard)
...
━━━━━━━━━━━━━━━━━━━━━━
[👨‍💻] 𝐃𝐞𝐯: <a href='tg://user?id=7593550190'>𓆰𝅃꯭᳚⚡!! ⏤͟𝐅ɴ x 𝐄ʟᴇᴄᴛʀᴀ𓆪𓆪⏤͟➤⃟🔥✘</a>"""
    )
    ...
    if hits:
        ...
        await update.message.reply_document(
            ...
            filename=hits_file,
            caption="🎉 𝐘𝐨𝐮𝐫 𝐇𝐢𝐭𝐬 𝐅𝐢𝐥𝐞 𝐈𝐬 𝐑𝐞𝐚𝐝𝐲! 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐍𝐨𝐰."
        )
        logger.info("200 OK: Hits file generated")
        os.remove(hits_file)
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) != 3 or args[0] not in CHECKING_LIMITS or not args[1].endswith("d") or not args[2].isdigit():
        ...
        logger.error("Invalid genkey arguments")
        return
    ...
    response = (
        f"🎁 𝐆𝐢𝐟𝐭 𝐊𝐞𝐲 𝐆𝐞𝐧𝐞𝐫𝐚𝐭𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲! 🎉\n\n"
        f"🔢 𝐐𝐮𝐚𝐧𝐭𝐢𝐭𝐲: {quantity}\n"
        f"📋 𝐃𝐞𝐭𝐚𝐢𝐥𝐬:\n" +
        "\n".join([f"🔑 `{key}`\n  └─ 💎 **Tier**: {tier} | ⏳ **Duration**: {duration} days" for key in keys]) +
        f"\n\n📌 𝐓𝐨 𝐑𝐞𝐝𝐞𝐞𝐦: 𝐔𝐬𝐞 `/redeem <key>`"
    )
    await update.message.reply_text(response, parse_mode="Markdown")
    logger.info(f"Generated {quantity} keys for tier {tier} by user {user_id}")
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not key_data:
        ...
        logger.error(f"Invalid or used key: {key}")
        return
...
𝐘𝐨𝐮𝐫 𝐏𝐫𝐞𝐦𝐢𝐮𝐦 𝐒𝐮𝐛𝐬𝐜𝐫𝐢𝐩𝐭𝐢𝐨𝐧 𝐈𝐬 𝐍𝐨𝐰 𝐀𝐜𝐭𝐢𝐯𝐚𝐭𝐞𝐝! ✅

💎 𝐓𝐢𝐞𝐫: {key_data['tier']}
⏳ 𝐃𝐮𝐫𝐚𝐭𝐢𝐨𝐧: {key_data['duration_days']} days
📅 𝐄𝐱𝐩𝐢𝐫𝐞𝐬 𝐎𝐧: {expiration.strftime('%Y-%m-%d %H:%M:%S')} UTC

𝐓𝐡𝐚𝐧𝐤𝐘𝐨𝐮 𝐅𝐨𝐫 𝐔𝐬𝐢𝐧𝐠 𝐅𝐍-𝐁3-𝐀𝐔𝐓𝐇 ❤️"""
    )
    logger.info(f"Key {key} redeemed by user {user_id} for tier {key_data['tier']}")
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users:
        try:
            ...
            continue
    await update.message.reply_text(f"📢 Broadcast sent to {sent_count} users successfully.")
