...
async def handle_proxies_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        file_content = await file.download_content()
...
async def button_callback(update: Update, context: ContextTypes.Update):
...
async def check_cc(cx: str, user_id: int, tier: str, context: ContextTypes.DEFAULT_TYPE) -> dict:
    ...
    try:
        ...
        try:
            ...
            async with COOKIE_LOCK:
                headers = {
                    "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/av,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = (
        ...
        '\n'.join([f"➔ {key}\n𝐕𝐚𝐥𝐮e: {tier} {duration} days" for key in keys]) +
        "\n\n𝐅𝐨𝐫 𝐑𝐞𝐝𝐞𝐦𝐩𝐭𝐢𝐨𝐧\n𝐓𝐲𝐩𝐞 /redeem {key}"
    )
