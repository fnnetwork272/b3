...
def get_proxies(num_proxies):
    ...
    # Cycle through available proxies repeatedly
...
def b3req(cc, mm, yy, proxy):
    ...
    logger.info(f"HTTP Request: POST https://payments.braintree-api.com/graphql - Status: {requests.post('https://payments.braintree-api.com/graphql', headers=headers, json=json_data, proxies=proxy, timeout=10).status_code}")
    try:
        response = requests.post('https://payments.braintree-api.com/graphql', headers=headers, json=json_data, proxies=proxy, timeout=10)
        logger.info(f"HTTP Response: POST https://payments.braintree-api.com/graphql - Status: {response.status_code}")
    except requests.exceptions.RequestException as e:
        logger.error(f"HTTP Error: POST https://payments.braintree-api.com/graphql - Status: {e.response.status_code if hasattr(e.response, 'status_code') else 'N/A'} - Error: {e}")
    ...
    try:
        ...
        logger.error(f"HTTP Error: POST https://payments.braintree-api.com/graphql - JSON Decode Error - Status: N/A - Error: {e}")
        return None, None, None, None, None, None, None, None
    if 'data' not in resjson or not resjson['data']:
        logger.error("HTTP Response: POST https://payments.braintree-api.com/graphql - Braintree response has no 'data' key or data is empty")
        return None, None, None, None, None, None, None, None
    try:
        ...
        logger.error(f"HTTP Error: POST https://payments.braintree-api.com/graphql - KeyError in response parsing - Error: {e}")
        return None, None, None, None, None, None, None, None
...
async def process_file(file_path, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(0, len(lines), batch_size):
        ...
        proxies = get_proxies(len(batch))  # Use available proxies repeatedly
...
━━━━━━━━━━━━━━━━━━━━━━
<b>[✪] 𝐂𝐡𝐚𝐫𝐠𝐞𝐝:</b> {stats['approved']}
<b>[❌] 𝐃𝐞𝐜𝐥𝗶𝗻𝗲𝗱:</b> {stats['declined']}
...
𝗧𝗶𝗺𝗲: {duration:.2f} seconds
𝗖𝗵𝗲𝗰𝐤𝗲𝗱 𝐁𝐲: <a href="{profile_link}">{user_name}</a> ({tier} {TIERS[tier]['emoji'] if tier in TIERS else ''})
"""
        await initial_msg.edit_text(response_message, parse_mode='HTML')
    ...
    for i in range(0, len(lines), batch_size):
        ...
        for result in results:
            ...
            if is_charged(final):
                stats['approved'] += 1
                response_text = "Insufficient Funds" if "insufficient funds" in final.lower() else "CHARGED 25$😈⚡"
                await context.bot.send_message(chat_id=update.effective_chat.id, text=f"<b>Approved✅</b> {full_card.split('|')[0]} - Response: {response_text}", parse_mode='HTML')
            else:
                stats['declined'] += 1
...
𝐌𝐞𝐬𝐬𝐚𝐠𝐞: 𝐓𝐡𝗶𝐬 𝐠𝗶𝗳𝘁 𝗰𝗼𝗱𝗲 𝗶𝘀 𝐀𝐥𝐫𝐞𝐚𝗱𝐲 𝐑𝐞𝗱𝐞𝐞𝗺𝐞𝗱 𝐛𝐲 𝐚𝗻𝗼𝘁𝗵𝗲�_r 𝘂𝘀𝗲𝗿.
"""
            await update.message.reply_text(response, parse_mode='HTML')
        else:
...
𝐌𝐞𝐬𝐬𝐚𝐠𝐞: 𝐂𝗼𝗻𝗴𝗿𝗮𝘁𝘇! 𝐘𝗼𝘂𝗿 𝐏𝗿𝗼𝘃𝗶𝗱𝗲𝗱 𝐆𝗶𝗳𝘁𝗰𝗼𝗱𝗲 𝐒𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝐑𝐞𝗱𝐞𝐞𝗺𝐞𝗱 𝘁𝗼 𝐘𝗼𝘂𝗿 𝐀𝗰𝗰𝗼𝘂𝗻𝘁, 𝐀𝗻𝗱 𝐘𝗼𝘂 𝐆𝗼𝘁 𝗮 "{key_data['tier']} 𝐅𝗼𝗿 {key_data['duration_days']} 𝐃𝐚𝐲𝘀".
"""
    await update.message.reply_text(response, parse_mode='HTML')
...
if __name__ == '__main__':
    if not load_proxies():
        logger.error("Proxies not set or expired. Please set valid proxies in proxiess.txt before continuing.")
        exit(1)
    country_mapping = load_countries()

    app = ApplicationBuilder().token('7620898782:AAFpTD0KXDqE9hYjObM9WEwGLDOtfHFo3C0').build()
