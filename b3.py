...
𝗧𝗶𝗺𝗈: {result['time_taken']:.2f} seconds
𝗖𝗵𝐞𝐜𝐤𝐞𝐝 𝐁𝐲: {checked_by}
"""
        ...
        for i in range(0, len(cards), chunk_size):
            ...
            for card, result in zip(chunk, results):
                if isinstance(result, Exception):
                    logger.error(f"Error checking card {card}: {str(result)}")
                    continue
            ...
            await asyncio.sleep(60)  # Wait 60 seconds after each batch
