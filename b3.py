...
COOLDOWN_SECONDS = 10  # 10-second cooldown between requests
LAST_REQUEST_TIME = 0
...
async def save_cookies():
    async with COOKIE_LOCK:
        try:
            logger.info("Attempting to save cookies to MongoDB...")
            update_task = cookies_collection.update_one(
                {"key": "session_cookies"},
                {"$set": {"cookies": SESSION_COOKIES, "updated_at": datetime.utcnow()}},
                upsert=True
            )
            await asyncio.wait_for(update_task, timeout=10)
            logger.info("Cookies saved to MongoDB successfully")
        except asyncio.TimeoutError:
            logger.error("MongoDB operation timed out while saving cookies")
            raise Exception("Failed to save cookies: MongoDB operation timed out")
        except Exception as e:
