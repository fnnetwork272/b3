        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://bins.antipublic.cc/bins/{bin_number}") as response:
                if response.status == 200:
                    ...
                    flag = data.get('flag', '')  # Use the flag directly from the API
                    return bank, country, card_type, flag
                else:
...
async def check_cc(cc_details):
    ...
    bin_number = cc[:6]
    issuer, country, card_type, flag = await get_bin_details(bin_number)
    ...
    try:
        ...
        result = {
            ...
            'proxy_status': 'Live' if PROXY and PROXY_LIST else 'None',
            'issuer': issuer,
            'country': country,
            'flag': flag,
            'card_type': card_type
...
async def single_check(user_id, cc_details, update, context):
    ...
    card_info = result['card_type']
    issuer = result['issuer']
    country = f"{result['country']} {result['flag']}"
    ...
    if result['status'] == 'approved':
        ...
        if '2010: Card Issuer Declined CVV' in result['message']:
            msg = msg.replace("𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝 ✖️", "𝐂𝐂𝐍 ✅")
        await update.message.reply_text(msg, parse_mode='HTML')
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'upload_files':
        ...
        await query.message.reply_text("Stopping the check... 🔴")
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, cc_details in enumerate(cc_list):
        ...
        # Updated progress format without progress bar
        progress_text = (
            f"🔎 𝐂𝐡𝐞𝐜𝐤𝐢𝐧𝐠 𝐂𝐚𝐫𝐝𝐬...\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"𝐑𝐞𝐬𝐩𝐨𝐧𝐬𝐞: {last_response}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"[み] 𝐁𝐨𝐭: @FN_B3_AUTH"
        )
        
        # Inline buttons for Approved, Declined, Total, and Stop
        keyboard = [
    ...
    if not stop_checking.get(user_id, False):
        ...
        # Add inline buttons with final stats
        keyboard = [
            [InlineKeyboardButton(f"✅ Approved: {approved}", callback_data='view_approved')],
            [InlineKeyboardButton(f"❌ Declined: {declined}", callback_data='view_declined')],
            [InlineKeyboardButton(f"💳 Total: {total}", callback_data='view_total')]
