...
active_tasks = set()  # Changed to set for cleaner task management
bulk_progress = {}  # To track bulk check progress
...
async def process_checks():
    while True:
        ...
        # Process new tasks up to concurrency limit
        while len(active_tasks) < 3 and not check_queue.empty():
            item = await check_queue.get()
            user_id = item[0]
            if stop_checking.get(user_id, False) and item[4]:  # Skip bulk tasks if stopped
                bulk_id = item[5]
                if bulk_id in bulk_progress:
                    async with bulk_progress[bulk_id]['lock']:
                        ...
                        if bulk_progress[bulk_id]['pending'] == 0:
                            await send_final_message(user_id, bulk_id, item[3])  # context
                continue
...
async def send_final_message(user_id, bulk_id, context):
    ...
    with open(hit_file, 'rb') as f:
        ...
        await context.bot.send_document(
            ...
            caption=(
                ...
                f"[✪] 𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝: {approved}\n"
                f"[❌] 𝐃𝐞𝐜𝐥𝐢𝐧𝐞𝐝: {declined}\n"
                f"[✪] 𝐂𝐡𝐞𝐜𝐤𝐞�{d}: {approved + declined}/{total}\n"
                f"[✪] 𝐓𝐨𝐭𝐚𝐥: {total}\n"
    ...
    os.remove(hit_file)
    del bulk_progress[bulk_id]  # Clean up
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for cc_details in cc_list:
        await check_queue.put((user_id, cc_details, update, context, True, bulk_id))
