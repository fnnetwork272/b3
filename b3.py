...
𝗧𝗶𝗺𝗲: {duration:.2f} seconds
𝗖𝗵𝗲𝗰𝗸𝗲𝗱 𝐁𝐲: <a href="{profile_link}">{user_name}</a> ({tier} {TIERS[tier]['emoji'] if tier in TIERS else ''})
"""
        await initial_msg.edit_text(response_message, parse_mode='HTML')
...
𝐌𝐞𝐬𝐬𝐚𝐠𝐞: 𝐂𝗼𝗻𝗴𝗿𝗮𝘁𝘇! 𝐘𝗼𝘂𝗿 𝐏𝗿𝗼𝘃𝗶𝗱𝗲𝗱 𝐆𝗶𝗳𝘁𝗰𝗼𝗱𝗲 𝐒𝘂𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝐑𝐞𝗱𝐞𝐞𝗺𝐞𝗱 𝘁𝗼 𝐘𝗼𝘂𝗿 𝐀𝗰𝗰𝗼𝘂𝗻𝘁, 𝐀𝗻𝗱 𝐘𝗼𝘂 𝐆𝗼𝘁 𝗮 "{key_data['tier']} 𝐅𝗼𝗿 {key_data['duration_days']} 𝐃𝐚𝘆𝘀".
"""
    await update.message.reply_text(response, parse_mode='HTML')
