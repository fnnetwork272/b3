...
def b3req(cc, mm, yy, proxy):
    ...
    logger.info(f"Braintree API Request - Status: {requests.post('https://payments.braintree-api.com/graphql', headers=headers, json=json_data, proxies=proxy, timeout=10).status_code} - URL: https://payments.braintree-api.com/graphql")
    try:
        response = requests.post('https://payments.braintree-api.com/graphql', headers=headers, json=json_data, proxies=proxy, timeout=10)
        logger.info(f"Braintree API Response - Status: {response.status_code} - URL: https://payments.braintree-api.com/graphql")
    except requests.exceptions.RequestException as e:
        logger.error(f"Braintree API Error - Status: {e.response.status_code if hasattr(e.response, 'status_code') else 'N/A'} - URL: https://payments.braintree-api.com/graphql - Error: {e}")
    ...
    try:
        ...
        logger.error(f"Braintree JSON Decode Error - Status: N/A - URL: https://payments.braintree-api.com/graphql - Error: {e}")
        return None, None, None, None, None, None, None, None
    if 'data' not in resjson or not resjson['data']:
        logger.error("Braintree response has no 'data' key or data is empty - Status: N/A - URL: https://payments.braintree-api.com/graphql")
        return None, None, None, None, None, None, None, None
    try:
        ...
        logger.error(f"Braintree KeyError in response parsing - Status: N/A - URL: https://payments.braintree-api.com/graphql - Error: {e}")
        return None, None, None, None, None, None, None, None
...
def brainmarkreq(b3tkn, mm, yy, bin, Type, lastfour, lasttwo, proxy):
    ...
    logger.info(f"Brandmark API Request - Status: {requests.post('https://app.brandmark.io/v3/charge.php', cookies=cookies2, headers=headers2, json=json_data2, proxies=proxy, timeout=10).status_code} - URL: https://app.brandmark.io/v3/charge.php")
    try:
        ...
        logger.info(f"Brandmark API Response - Status: {response2.status_code} - URL: https://app.brandmark.io/v3/charge.php")
    except requests.exceptions.RequestException as e:
        logger.error(f"Brandmark API Error - Status: {e.response.status_code if hasattr(e.response, 'status_code') else 'N/A'} - URL: https://app.brandmark.io/v3/charge.php - Error: {e}")
    ...
    try:
        ...
        logger.error(f"Brandmark JSON Decode or KeyError - Status: N/A - URL: https://app.brandmark.io/v3/charge.php - Error: {e}")
...
<b>[⌬]CHECKED BY -»</b> <a href="{profile_link}">{user_name}</a> ({tier} {TIERS[tier]['emoji'] if tier in TIERS else ''})
...
𝗖𝗵𝗲𝗰𝗸𝗲𝗱 𝗕𝘆: <a href="{profile_link}">{user_name}</a> ({tier} {TIERS[tier]['emoji'] if tier in TIERS else ''})
"""
...
𝐌𝐞𝐬𝐬𝐚𝐠𝐞: 𝐓𝐡𝐢𝐬 𝐠𝐢𝐟𝐭 𝐜𝐨𝐝𝐞 𝐢𝐬 𝐀𝐥𝐫𝐞𝐚𝐝𝐲 𝐑𝐞𝐝𝐞𝐞𝗺𝐞𝐝 𝐛𝐲 𝐚𝐧𝐨𝐭𝐡𝐞𝐫 𝐮𝐬𝐞𝐫.
"""
            await update.message.reply_text(response, parse_mode='HTML')
        else:
...
𝐌𝐞𝐬𝐬𝐚𝐠𝐞: 𝐂𝐨𝐧𝐠𝐫𝐚𝐭𝐳! 𝐘𝐨𝐮𝐫 𝐏𝐫𝐨𝐯𝐢𝐝𝐞𝐝 𝐆𝐢𝐟𝐭𝐜𝗼𝗱𝗲 𝐒𝐮𝗰𝗰𝗲𝘀𝘀𝗳𝘂𝗹𝗹𝘆 𝐑𝐞𝐝𝐞𝗲𝗺𝐞𝐝 𝐭𝐨 𝐘𝐨𝐮𝐫 𝐀𝗰𝗰𝗼𝘂𝗻𝘁, 𝐀𝐧𝐝 𝐘𝐨𝐮 𝐆𝗼𝘁 𝐚 "{key_data['tier']} 𝐅𝐨𝐫 {key_data['duration_days']} 𝐃𝐚𝐲𝐬".
"""
    await update.message.reply_text(response, parse_mode='HTML')
