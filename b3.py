        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://bins.antipublic.cc/bins/{bin_number}") as response:
                if response.status == 200:
                    ...
                    country = data.get('country', 'Unknown')  # Full country name from API
                    card_type = data.get('brand', 'Unknown').capitalize()  # Capitalize for consistency (e.g., "Visa")
                    flag = data.get('flag', '')  # Flag emoji from API
...
async def single_check(user_id, cc_details, update, context):
    ...
    country = f"{result['country']} {result['flag']}"  # Use full country name with flag
...
async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, cc_details in enumerate(cc_list):
        ...
        if result['status'] == 'approved':
            ...
            approved_msg = (f"𝐀𝐩𝐩𝐫o𝐯𝐞𝐝 ✅\n\n"
                           f"[ϟ]𝗖𝗮𝗿𝗱 -» <code>{result['card']}</code>\n"
                           f"[ϟ]𝗚𝗮𝘁𝗲𝘄𝗮𝘆 -» Braintree Auth\n"
                           f"[ϟ]𝗥𝗲𝘀𝗽𝐨𝐧𝐬𝐞 -» Approved ✅\n\n"
                           f"[ϟ]𝗜𝗻𝗳𝐨 -» {card_info}\n"
                           f"[ϟ]𝗜𝘀𝘀𝘂𝗲𝗿 -» {issuer} 🏛\n"
                           f"[ϟ]𝗖𝐨𝐮𝐧𝐭𝐫𝐲 -» {country}\n\n"
                           f"[⌬]𝗧𝗶𝗺𝐞 -» {result['time_taken']:.2f} seconds\n"
                           f"[⌬]𝗣𝗿𝐨𝐱𝐲 -» {result['proxy_status']}\n"
                           f"[⌬]𝗖𝗵𝐞𝐜𝐤𝐞𝐝 𝐁𝐲 -» {checked_by} {tier}\n"
                           f"[み]𝗕𝐨𝐭 -» <a href='tg://user?id=8009942983'>𝙁𝙉 𝘽3 𝘼𝙐𝙏𝙃</a>")
