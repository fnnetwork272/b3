...
async def check_cc(cc, mes, ano, cvv, proxy=None, semaphore=None):
...
async def process_single_check(update, context, user_id, chat_id, chat_type, card):
    try:
        ...
        result = await check_cc(cc, mes, ano, cvv, proxy, user_semaphore)
            ...
            for card in chunk:
                try:
                    ...
                    if len(ano) == 2:
                        ano = f'20{ano}'
                    proxy = random.choice(proxies) if proxies and USE_PROXIES else None
                    tasks.append(check_cc(cc, mes, ano, cvv, proxy, user_semaphore))
                except Exception as e:
